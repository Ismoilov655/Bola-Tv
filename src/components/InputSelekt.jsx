...
const InputSelekt = () => {
    ...
    const scroll = (direction, category) => {
        ...
        if (scrollContainer) {
            scrollContainer.scrollLeft += direction === "left" ? -350 : 350;
        }
    };
        ...
        <>
            <section className='w-full max-w-7xl mx-auto px-5 mb-10 md:mb-16'>
                <form onSubmit={(e) => e.preventDefault()} className='flex items-center justify-end sm:justify-between'>
                    <input value={query}
                        onChange={(e) => setQuery(e.target.value)} id='name-imput' className='hidden sm:block w-3/5 text-base bg-transparent border-b border-br-darkBlue focus:outline-none mr-10 md:text-lg md:w-1/2' type="text" />

                    <select value={language || "default"}
                        onChange={(e) => {
                            const newLang = e.target.value;
                            setLanguageState(newLang);
                            setLanguage(newLang);
                            langRef.current.textContent = e.target.value;
                        }}
                        className='class="p-1 rounded-sm border-2 border-dotted border-br-green focus:outline-none sm:p-2 md:w-36"'>
            ...
            {Object.keys(filteredData).map((category) => (
                <section key={category} className="w-full max-w-7xl mx-auto px-5 mb-8 md:mb-10">
                    ...
                    <div ref={(el) => (scrollRefs.current[category] = el)} className="w-full flex md:grid md:grid-cols-2 lg:grid-cols-3 gap-4 overflow-hidden whitespace-nowrap"
                    >
                        {filteredData[category]?.map((item) => (
                            <div onClick={() => setSelectedItem(item)} key={item.id} className="w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg relative group pb-3 rounded-xl transition-all duration-300 cursor-pointer border-2 shadow-md bg-white flex-shrink-0"
                            >
            ...
            {selectedItem && (
                <div className="bg-black/50 w-screen h-screen fixed top-0 left-0 z-40" onClick={() => setSelectedItem(null)}>
                    <div className="w-full h-[250px] md:h-[600px] bg-white rounded-lg z-20 fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 md:max-w-5xl" onClick={(e) => e.stopPropagation()}>
